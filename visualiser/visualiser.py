...
class Visualiser(object):
    ...
    def write_gif(cls, name="out.gif", delay=3):
        ...
        img_sorted = sorted(imgs,
                            key=lambda fn: int(fn.split("_")[1].split(".")[0]))
        for filename in img_sorted:
    ...
    def extract_signature_label_arg_string(self, *args, **kwargs):
        """
        Returns function signature arguments as string and function label
        arguments as string.

        label_argument string contains only the arguments that are not
            in ignore_args
        signature_argument_string contains all the arguments available
            for the function
        """
        # If show_argument flag is True(default)
        # Then argument_string is:
        # a=1, b=31, c=0
        signature_argument_string = ', '.join(
            [repr(a) for a in args] +
            [f"{key}={repr(value)}" for key, value in kwargs.items()]
        )

        label_argument_string = ', '.join(
            [repr(a) for a in args] +
            [f"{key}={repr(value)}" for key, value in kwargs.items()
    ...
    def __call__(self, fn):
        ...
        def wrapper(*args, **kwargs):
            ...
            current = self.extract_signature_label_arg_string(*args, **kwargs)
            current_function_argument_string, current_function_label_argument_string = current
            ...
            current_function_label = f"{current_function_name}(" \
                f"{current_function_label_argument_string})"
            """"""
            ...
            caller_function_locals = OrderedDict(sorted(
                caller_function_locals.items()))
            # Extract only those locals that are in arguments
            caller_function_kwargs = {
                key: value for key, value in caller_function_locals.items()
                if key in caller_function_argument_names
            }

            # If the nodes has parent node get node_num from parent node
            if self.stack:
                caller_function_kwargs.update({'node_num': self.stack[-1]})
            caller_function_kwargs = OrderedDict(sorted(
                caller_function_kwargs.items()))

            caller = self.extract_signature_label_arg_string(
                **caller_function_kwargs)
            caller_function_argument_string, caller_function_label_argument_string = caller

            # Caller Function
            caller_function_name = caller_function_frame.f_code.co_name

            # Extract the names of arguments only
            caller_func_signature = f"{caller_function_name}(" \
                                    f"{caller_function_argument_string})"
            caller_func_label = f"{caller_function_name}(" \
                                f"{caller_function_label_argument_string})"
            """"""
            ...
            self.edges.append(f'"{caller_func_signature}" -> '
                              f'"{current_function_signature}"')

            # Construct node string to be rendered in graphviz
            node_string = f'"{current_function_signature}" ' \
                          f'[label="{current_function_label}"'
            ...
            if self.node_properties_kwargs:
                ...
                node_string += ", " + ", ".join(node_properties)
            ...
            child_node = pydot.Node(name=current_function_signature,
                                    label=current_function_label,
